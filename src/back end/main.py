...
from recommender import recommend, recommend_w_rf, build_song_clusters
...
global_clusters =  build_song_clusters(song_dict,300)
...
def make_recommendation(random_forest=False,
   ...
   # song_dict = read_song_dict_w_labels()
    if random_forest:
        ...
        return jsonify(recommend_w_rf(song_dict, entered_songs, rec_amount, global_clusters))
    else:
        if entered_songs is None:
            entered_songs = get_rnd_entered_songs(song_dict, entered_amount)
        return jsonify(recommend(song_dict, entered_songs, rec_amount, global_clusters))
...
#    print(search_songs(artist, title, song_id, eligibility))
